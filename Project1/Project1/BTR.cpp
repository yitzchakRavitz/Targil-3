...
template <typename T>
T* BTree<T>::search(BNode<T> *p, T x)//the function accepts a value in the tree and returns a pointer to the place in the tree
{
	if (p == NULL)//stop conditions
		return NULL;
	if (p->indexValInK(x) == -1)//the function indexValInK returns -1 if the value is not found
	{
		for (int j = 0; j < p->nsons; j++)
		{
			search(p->Son[j], x);
		}
	}
	...
	{
		T* a = &(p->Key[p->indexValInK(x)]);//return the pointer
		return a;
	}
...
template <typename T>
void BTree<T>::printAllKeys(BNode<T> *p, std::function<bool(const T&)> predicate)//print the All Keys in the tree according to the condition in the function
{
	if (p == NULL)//stop conditions
	{
		return;
	...
	for (int i = 0; i < p->nkeys; i++)
	{
		...
		if (predicate(p->Key[i]) == true)//checks whether the condition of the function is correct
		{
			cout << p->Key[i] << endl;
		}
	}
...
template <typename T>
void BTree<T>::deleteSubTree(BNode<T> *t)//the function gets a pointer to the leaf and prints the entire tree from it
{
	if (t == NULL)
	{
		return;
...
template <typename T>
void BTree<T>::printSubTree(BNode<T> *t)//print the Tree from the node 't' to the ond of the tree
{
	if (t == NULL)
	{
		return;
